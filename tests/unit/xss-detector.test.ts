...
describe('XssDetector', () => {
  ...
  describe('getContextAwarePayloads', () => {
    it('prioritizes an execution proof for hash-route parameters', () => {
      ...
      const payloads = (detector as any).getContextAwarePayloads(
        surface,
        'none',
        'none'
      ) as string[];

      expect(payloads[0]).toContain('<iframe src="javascript:alert');
      expect(payloads[1]).toContain('<iframe src="javascript:window.kinetic_proof');
    });
  });
});
